...
#    def write_netcdf_output(output_fullfn, modelparameters, mb_mwea, observed_massbal):
#            """
...
#            ds.close()
    ...
    for glac in range(main_glac_rgi.shape[0]):
        ...
        # TESTING: Record output to xarray dataset
    ...
    #                      'offglac ref max:', np.round(offglac_wide_refreeze.max(),2))
            ...
            output_refreeze_glac_monthly = calc_stats_array(output_refreeze_glac_monthly)
            output_melt_glac_monthly = calc_stats_array(output_melt_glac_monthly)
            output_frontalablation_glac_monthly = calc_stats_array(output_frontalablation_glac_monthly)
            output_massbaltotal_glac_monthly = calc_stats_array(output_massbaltotal_glac_monthly)
            output_runoff_glac_monthly = calc_stats_array(output_runoff_glac_monthly)
            output_snowline_glac_monthly = calc_stats_array(output_snowline_glac_monthly)
            output_area_glac_annual = calc_stats_array(output_area_glac_annual)
            output_volume_glac_annual = calc_stats_array(output_volume_glac_annual)
            output_ELA_glac_annual = calc_stats_array(output_ELA_glac_annual)
            output_offglac_prec_monthly = calc_stats_array(output_offglac_prec_monthly)
            output_offglac_refreeze_monthly = calc_stats_array(output_offglac_refreeze_monthly)
            output_offglac_melt_monthly = calc_stats_array(output_offglac_melt_monthly)
            output_offglac_snowpack_monthly = calc_stats_array(output_offglac_snowpack_monthly)
            output_offglac_runoff_monthly = calc_stats_array(output_offglac_runoff_monthly)
            ...
            output_ds_all_stats['prec_glac_monthly'].values[0,:,:] = output_prec_glac_monthly_stats
            output_ds_all_stats['temp_glac_monthly'].values[0,:,:] = output_temp_glac_monthly_stats
            output_ds_all_stats['acc_glac_monthly'].values[0,:,:] = output_acc_glac_monthly_stats
            output_ds_all_stats['refreeze_glac_monthly'].values[0,:,:] = output_refreeze_glac_monthly
            output_ds_all_stats['melt_glac_monthly'].values[0,:,:] = output_melt_glac_monthly
            output_ds_all_stats['frontalablation_glac_monthly'].values[0,:,:] = output_frontalablation_glac_monthly
            output_ds_all_stats['massbaltotal_glac_monthly'].values[0,:,:] = output_massbaltotal_glac_monthly
            output_ds_all_stats['runoff_glac_monthly'].values[0,:,:] = output_runoff_glac_monthly
            output_ds_all_stats['snowline_glac_monthly'].values[0,:,:] = output_snowline_glac_monthly
            output_ds_all_stats['area_glac_annual'].values[0,:,:] = output_area_glac_annual
            output_ds_all_stats['volume_glac_annual'].values[0,:,:] = output_volume_glac_annual
            output_ds_all_stats['ELA_glac_annual'].values[0,:,:] = output_ELA_glac_annual
            output_ds_all_stats['offglac_prec_monthly'].values[0,:,:] = output_offglac_prec_monthly
            output_ds_all_stats['offglac_refreeze_monthly'].values[0,:,:] = output_offglac_refreeze_monthly
            output_ds_all_stats['offglac_snowpack_monthly'].values[0,:,:] = output_offglac_snowpack_monthly
            ...
            #%%
            # testing
#            ds = xr.open_dataset(input.main_directory + '/../Output/simulations/CCSM4/13.00001_CCSM4_rcp26_c2_ba1_100sets_2000_2100.nc')
...
#        write_netcdf_output(netcdf_output_fp + glacier_str + '.nc', modelparameters, mb_mwea, observed_massbal)
...
#                         str(input.option_bias_adjustment) + '_' +  str(sim_iters) + 'sets' + '_' + 
#                         str(input.gcm_startyear) + '_' + str(input.gcm_endyear) + '--' + str(count) + '.nc')
#        else:
#            netcdf_fn = (regions_str + '_' + gcm_name + '_' + rcp_scenario + '_c' + 
#                         str(input.option_calibration) + '_ba' + str(input.option_bias_adjustment) + '_' +  
#                         str(sim_iters) + 'sets' + '_' + str(input.gcm_startyear) + '_' + str(input.gcm_endyear) + 
#                         '--' + str(count) + '.nc')
#        if input.option_synthetic_sim==1:
...
#    print('DELETE ME!')
    BATMAN_fp = input.output_sim_fp + 'Batman/'
#    # Create filepath if it does not exist
#    if os.path.exists(BATMAN_fp) == False:
#        os.makedirs(BATMAN_fp)
...
#%% TRYING TO USE THE CALIBRATION INPUT TO SEE IF THAT'S BETTER!     
#if __name__ == '__main__':
#    time_start = time.time()
#    parser = getparser()
#    args = parser.parse_args()
