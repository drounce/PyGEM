    ...
    if icethickness_t0.max() > 0:    
        ...
        for year in range(0, annual_columns.shape[0]):  
            ...
            if (input.timestep == 'monthly') and (glac_idx_t0.shape[0] != 0):      
                ...
                if (input.option_calibration == 1) or (year < input.spinupyears):
        ...
        print('Glacier ',glac,'did not have any ice thickness')
...
#    precipitation in each bin based on the temperature threshold (temp_snow).
#    Output: numpy array of precipitation [m] and accumulation [m w.e.] in each bin (rows = bins, columns = dates)
#    """
#    # option_prec2bins:
