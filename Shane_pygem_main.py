...
def getparser():
    ...
    parser.add_argument('--RGIId', type=str, default='0',
                        help='RGIId (ex. RGI60-15.03473)')
    parser.add_argument('--glacierlist', type=str, default='0', 
...
def main(RGIId):
    ...
    # LOAD DATA 
    # ===== RGI INFO =====
    if os.path.exists(input.modelsetup_dir + RGIId + '_rgi_table.pk') == True:
        glacier_rgi_table = pd.read_pickle(input.modelsetup_dir + RGIId + '_rgi_table.pk')
        icethickness_t0 = pd.read_pickle(input.modelsetup_dir + RGIId + '_icethickness.pk')
        glacier_area_t0 = pd.read_pickle(input.modelsetup_dir + RGIId + '_glacierarea.pk')
        width_t0 = pd.read_pickle(input.modelsetup_dir + RGIId + '_width.pk')
        elev_bins = np.load(input.modelsetup_dir + 'elevbins.pk')
        glacier_gcm_temp = np.load(input.modelsetup_dir + RGIId + '_gcmtemp.pk')
        glacier_gcm_prec = np.load(input.modelsetup_dir + RGIId + '_gcmprec.pk')
        glacier_gcm_elev = np.load(input.modelsetup_dir + RGIId + '_gcmelev.pk')
        glacier_gcm_lrgcm = np.load(input.modelsetup_dir + RGIId + '_gcmlr.pk')
...
#        print('Imported pickled data')
        ...
        dates_table, start_date, end_date = modelsetup.datesmodelrun(
                startyear=input.startyear, endyear=input.endyear, spinupyears=input.spinupyears)
    
        # PICKLE DATA
        with open(input.modelsetup_dir + RGIId + '_rgi_table.pk', 'wb') as pickle_rgi:
            # dump your data into the file
            pickle.dump(glacier_rgi_table, pickle_rgi)
#            print('Data pickled')
        with open(input.modelsetup_dir + RGIId + '_icethickness.pk', 'wb') as pickle_icethickness:
            # dump your data into the file
            pickle.dump(icethickness_t0, pickle_icethickness)
        with open(input.modelsetup_dir + RGIId + '_glacierarea.pk', 'wb') as pickle_glacierarea:
            # dump your data into the file
            pickle.dump(glacier_area_t0, pickle_glacierarea)    
        with open(input.modelsetup_dir + RGIId + '_width.pk', 'wb') as pickle_width:
            # dump your data into the file
        ...
        with open(input.modelsetup_dir + RGIId + '_gcmtemp.pk', 'wb') as pickle_gcmtemp:
            # dump your data into the file
            pickle.dump(glacier_gcm_temp, pickle_gcmtemp)
        with open(input.modelsetup_dir + RGIId + '_gcmprec.pk', 'wb') as pickle_gcmprec:
            # dump your data into the file
            pickle.dump(glacier_gcm_prec, pickle_gcmprec)
        with open(input.modelsetup_dir + RGIId + '_gcmelev.pk', 'wb') as pickle_gcmelev:
            # dump your data into the file
            pickle.dump(glacier_gcm_elev, pickle_gcmelev)
        with open(input.modelsetup_dir + RGIId + '_gcmlr.pk', 'wb') as pickle_gcmlr:
            # dump your data into the file
...
if __name__ == "__main__":
    ...
    elif args.glacierlist != '0':
        glacierlist = np.genfromtxt(args.glacierlist, dtype='str', delimiter=',')
        for glac in range(glacierlist.shape[0]):
            print(glacierlist[glac])
            RGIId = glacierlist[glac]
            (glacier_rgi_table, icethickness_t0, glacier_area_t0, width_t0, elev_bins, glacier_gcm_temp, 
            glacier_gcm_prec, glacier_gcm_elev, glacier_gcm_lrgcm, glacier_gcm_lrglac, dates_table, 
            glac_wide_massbaltotal, glac_wide_runoff, glac_wide_snowline, glac_wide_snowpack, glac_wide_area_annual, 
            glac_wide_volume_annual, glac_wide_ELA_annual, modelparameters, glac_wide_massbaltotal_annual_avg,
