...
class PyGEMMassBalance(MassBalanceModel):
    ...
    def __init__(
        ...
        if pygem_prms["mb"]["option_refreezing"] == "HH2015":
            ...
            self.rf_layers_ch = (1 - self.rf_layers_dens / 1000) * pygem_prms[
                "constants"
            ]["ch_air"] + self.rf_layers_dens / 1000 * pygem_prms["constants"][
                "ch_ice"
            ]
            self.rf_layers_k = (1 - self.rf_layers_dens / 1000) * pygem_prms[
                "constants"
            ]["k_air"] + self.rf_layers_dens / 1000 * pygem_prms["constants"][
                "k_ice"
            ]
    ...
    def get_annual_mb(
        ...
        if pygem_prms["mb"]["option_refreezing"] == "HH2015" and year == 0:
            self.te_rf[:, :, 0] = (
                0  # layer temp of each elev bin for present time step
            )
            self.tl_rf[:, :, 0] = (
        ...
        if self.glacier_area_initial.sum() > 0:
            ...
            if year == 0:
                self.surfacetype, self.firnline_idx = (
                    self._surfacetypebinsinitial(self.heights)
                )
            ...
            if option_areaconstant == False:
                ...
                offglac_idx = np.where(
                    self.offglac_bin_area_annual[:, year] > 0
                )[0]
            ...
            if pygem_prms["time"]["timestep"] == "monthly":
                ...
                if pygem_prms["mb"]["option_preclimit"] == 1:
                    ...
                    if len(fl.surface_h[fl.widths_m > 0]):
                        raw_min_elev.append(
                            fl.surface_h[fl.widths_m > 0].min()
                        )
                        raw_max_elev.append(
                            fl.surface_h[fl.widths_m > 0].max()
                        )
                    ...
                    if elev_range > 1000:
                        # Indices of upper 25%
                        glac_idx_upper25 = glac_idx_t0[
                            heights[glac_idx_t0] >= elev_75
                        ]
                        ...
                        # exponential decay
                        bin_precsnow[
                            glac_idx_upper25, 12 * year : 12 * (year + 1)
                        ] = (
                            bin_precsnow[
                                glac_idx_75, 12 * year : 12 * (year + 1)
                            ]
                            * np.exp(
                                -1
                ...
                for month in range(0, 12):
                    ...
                    if step == 0:
                        ...
                        self.bin_snowpack[:, step] = (
                            self.snowpack_remaining[:, step - 1]
                            + self.bin_acc[:, step]
                        )
                    ...
                    for surfacetype_idx in self.surfacetype_ddf_dict:
                        self.surfacetype_ddf[
                            self.surfacetype == surfacetype_idx
                        ...
                        if (
                            surfacetype_idx == 1
                            and pygem_prms["mb"]["include_debris"]
                        ):
                    ...
                    if pygem_prms["mb"]["option_refreezing"] == "HH2015":
                        ...
                        for nbin, gidx in enumerate(glac_idx_t0):
                            ...
                            if (
                                ...
                                if (
                                    ...
                                    print(
                                        "\nMonth "
                                        + str(
                                            self.dates_table.loc[step, "month"]
                                        ),
                                ...
                                for h in np.arange(
                                    ...
                                    for j in np.arange(
                                        ...
                                        self.tl_rf[0, gidx, step] = (
                                            self.bin_temp[gidx, step]
                                        )
                                        ...
                                        self.te_rf[j, gidx, step] = self.tl_rf[
                                            ...
                                            j
                                        ] / self.rf_layers_ch[j] / pygem_prms[
                                            "mb"
                                        ]["HH2015_rf_opts"][
                                            "rf_dz"
                                        ] ** 2 * 0.5 * (
                            ...
                            if (
                                ...
                                print(
                                    ...
                                    "Rain:",
                                    np.round(
                                        self.bin_prec[glac_idx_t0[nbin], step],
                                        2,
                                    ),
                                    "Rfrz:",
                        ...
                        if self.debug_refreeze:
                            print(
                                ...
                                "Refreeze potential:",
                                np.round(
                                    refreeze_potential[glac_idx_t0[0]], 3
                                ),
                                "Snow depth:",
                                np.round(
                                    self.bin_snowpack[glac_idx_t0[0], step], 2
                                ),
                                "Snow melt:",
                                np.round(
                                    self.bin_meltsnow[glac_idx_t0[0], step], 2
                                ),
                                "Rain:",
                                np.round(
                                    self.bin_prec[glac_idx_t0[0], step], 2
                                ),
                            )
                    ...
                    self.glac_bin_refreeze[glac_idx_t0, step] = (
                        self.bin_refreeze[glac_idx_t0, step]
                    )
                    self.glac_bin_snowpack[glac_idx_t0, step] = (
                        self.bin_snowpack[glac_idx_t0, step]
                    )
                    ...
                    if option_areaconstant == False:
                        ...
                        self.offglac_bin_prec[offglac_idx, step] = (
                            self.bin_prec[offglac_idx, step]
                        )
        ...
        if len(glac_idx_t0) > 3:
            ...
            mb_filled[(mb_filled == 0) & (heights < height_max)] = (
                ...
                * (
                    height_min
                    - heights[(mb_filled == 0) & (heights < height_max)]
                )
            )
    ...
    def _convert_glacwide_results(
        ...
        if len(glac_idx) > 0:
            ...
            self.glac_wide_refreeze[12 * year : 12 * (year + 1)] = (
                self.glac_bin_refreeze[:, 12 * year : 12 * (year + 1)][
                    glac_idx
                ]
                * glacier_area_monthly[glac_idx]
            ).sum(0)
            ...
            snow_mask[
                self.glac_bin_snowpack[:, 12 * year : 12 * (year + 1)] > 0
            ] = 1
            ...
            try:
                ...
                snowline_idx = np.zeros(
                    (heights_monthly_wsnow.shape[1])
                ).astype(int)
                snowline_idx_nan = []
        ...
        if option_areaconstant == False and len(offglac_idx) > 0:
            ...
            self.offglac_wide_melt[12 * year : 12 * (year + 1)] = (
                self.offglac_bin_melt[:, 12 * year : 12 * (year + 1)][
                    offglac_idx
                ]
                * offglacier_area_monthly[offglac_idx]
            ).sum(0)
            ...
            self.offglac_wide_refreeze[12 * year : 12 * (year + 1)] = (
                self.offglac_bin_refreeze[:, 12 * year : 12 * (year + 1)][
                    offglac_idx
                ]
                * offglacier_area_monthly[offglac_idx]
            ).sum(0)
            ...
            self.offglac_wide_snowpack[12 * year : 12 * (year + 1)] = (
                self.offglac_bin_snowpack[:, 12 * year : 12 * (year + 1)][
                    offglac_idx
                ]
                * offglacier_area_monthly[offglac_idx]
            ).sum(0)
    ...
    def ensure_mass_conservation(self, diag):
        ...
        vol_change_annual_dif = (
            vol_change_annual_diag - vol_change_annual_mbmod
        )
        ...
        vol_change_annual_melt_reduction = np.zeros(
            vol_change_annual_mbmod.shape
        )
    ...
    def _surfacetypebinsannual(
        ...
        surfacetype[
            (surfacetype != 0) & (massbal_clim_mwe_runningavg <= 0)
        ] = 1
