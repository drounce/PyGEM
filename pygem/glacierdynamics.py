...
class MassRedistributionCurveModel(FlowlineModel):
    ...
    def run_until(self, y1, run_single_year=False):
        ...
        if self.check_for_boundaries:
            if self.fls[-1].thick[-1] > 10:
                raise RuntimeError(
                    "Glacier exceeds domain boundaries, at year: {}".format(
                        self.yr
                    )
                )
        # Check for NaNs
    ...
    def run_until_and_store(
        ...
        cyrs, cmonths = utils.hydrodate_to_calendardate(
            yrs, months, start_month=sm
        )
        ...
        diag_ds.attrs["creation_date"] = strftime(
            "%Y-%m-%d %H:%M:%S", gmtime()
        )
        ...
        diag_ds["volume_bsl_m3"].attrs["description"] = (
        ...
        for s, w, b in zip(sects, widths, bucket):
            ...
            ds["ts_section"] = xr.DataArray(
                s, dims=("time", "x"), coords=varcoords
            )
            ds["ts_width_m"] = xr.DataArray(
                w, dims=("time", "x"), coords=varcoords
            )
            if self.is_tidewater:
        ...
        if run_path is not None:
            ...
            for i, ds in enumerate(run_ds):
                ds.to_netcdf(
                    run_path, "a", group="fl_{}".format(i), encoding=encode
                )
            ...
            # MASS REDISTRIBUTION
                ...
                if self.is_tidewater:
                    ...
                    if debug:
                        ...
                        vol_init = (
                            self.fls[fl_id].section * fl.dx_meter
                        ).sum()
        ...
        if glacier_area_t0.sum() > 0:
            ...
            if last_above_wl is not None:
                if fl.bed_h[last_above_wl] < self.water_level:
                    ...
                    if (
                        q_calving
                        > q_calving_max + pygem_prms["constants"]["tolerance"]
                    ):
    ...
    def _massredistributionHuss(
        ...
        while glacier_volumechange_remaining < 0:
            ...
            glacier_area_t0_retreated = (
                width_t0_retreated * self.fls[0].dx_meter
            )
        ...
        while (
            ...
            glacier_area_t1 = (
                self.fls[0].widths_m.copy() * self.fls[0].dx_meter
            )

            # Advance volume [m3]
            advance_volume = (
                glacier_area_t0_raw[glac_idx_advance]
                * thick_t0_raw[glac_idx_advance]
            ).sum() - (
                glacier_area_t1[glac_idx_advance]
                * self.fls[0].thick[glac_idx_advance]
            ).sum()
            ...
            if self.fls[0].surface_h[glac_idx_t0_term] < self.water_level:
                ...
                if len(glac_idx_t0) > 2:
                    ...
                    elev_term = elev_sorted[1] - abs(
                        elev_sorted[2] - elev_sorted[1]
                    )
                else:
            ...
            if advance_volume > 0:
                ...
                try:
                    minelev_idx = np.where(
                        heights == heights[glac_idx_terminus].min()
                    )[0][0]
                    ...
                    glac_idx_terminus_removemin = list(
                ...
                if (
                    self.fls[0].thick[glac_idx_bin2add]
                    > terminus_thickness_avg
                ):
                    self.fls[0].thick[glac_idx_bin2add] = (
                        terminus_thickness_avg
                    )
                    ...
                    volume_added2bin = (
                        self.fls[0].section[glac_idx_bin2add]
                        * self.fls[0].dx_meter
                    )
            ...
            if advance_volume > 0:
                ...
                below_glac_idx = np.where(
                    heights < heights[glac_idx_t1].min()
                )[0]
                ...
                # otherwise, redistribute mass
                    ...
                    glac_bin_massbalclim_annual = np.zeros(
                        self.fls[0].thick.shape
                    )
    ...
    def _massredistributioncurveHuss(
        ...
        if glac_idx_t0.shape[0] > 3:
            ...
            fs_huss = (
                glacier_volumechange
                / (glacier_area_t0 * icethicknesschange_norm).sum()
            )
            if debug:
                print("fs_huss:", fs_huss)
            # Volume change [m3 ice]
            bin_volumechange = (
                icethicknesschange_norm * fs_huss * glacier_area_t0
            )
        ...
        self.fls[0].section = utils.clip_min(
            self.fls[0].section + section_change, 0
        )
        ...
        bin_volumechange_remaining[
            abs(bin_volumechange_remaining)
