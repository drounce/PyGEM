...
def prec_biasadj_HH2015(
    ...
    gcm_prec_biasadj_frac = gcm_prec_biasadj_subset.sum(
        axis=1
    ) / ref_prec_nospinup.sum(axis=1)
    assert (
        np.min(gcm_prec_biasadj_frac) > 0.5
        and np.max(gcm_prec_biasadj_frac) < 2
    ), (
...
def prec_biasadj_opt1(
    ...
    gcm_prec_annual_norm = (
        gcm_prec_annual / gcm_prec_annual.mean(1)[:, np.newaxis]
    )
    gcm_prec_annual_norm_repeated = np.repeat(
        gcm_prec_annual_norm, 12
    ).reshape(gcm_prec_biasadj.shape)
    ...
    for j in range(0, len(bc_temp)):
        ...
        for i in range(0, math.ceil(bc_loops)):
            ...
            for ival, projected_value in enumerate(bc_temp_loop):
                ...
                bc_temp_loop_corrected[ival] = (
                    projected_value * bias_correction_factor
                )
...
def monthly_avg_array_rolled(
    ...
    # Roll months so they are aligned with simulation months
    roll_amt = -1 * (12 - gcm_subset_idx_start % 12)
    ref_array_monthly_avg = np.roll(
        monthly_avg_2darray(ref_array), roll_amt, axis=1
    )
