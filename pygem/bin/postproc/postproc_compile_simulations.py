...
def run(args):
    ...
    for nbatch, glacno_list in enumerate(glacno_list_batches):
        ...
        main_glac_rgi_batch = main_glac_rgi_all.loc[
            main_glac_rgi_all.apply(
                lambda x: x.rgino_str in glacno_list, axis=1
            )
        ]
            ...
            for realization in realizations:
                ...
                for i, glacno in enumerate(glacno_list):
                    ...
                    try:
                        ...
                        # get monthly vars
                        glac_runoff_monthly = (
                            ds_glac.glac_runoff_monthly.values
                        )
                        offglac_runoff_monthly = (
                            ds_glac.offglac_runoff_monthly.values
                        )
                        # try extra vars
                        try:
                            glac_acc_monthly = ds_glac.glac_acc_monthly.values
                            glac_melt_monthly = (
                                ds_glac.glac_melt_monthly.values
                            )
                            glac_refreeze_monthly = (
                                ds_glac.glac_refreeze_monthly.values
                            )
                            ...
                            glac_prec_monthly = (
                                ds_glac.glac_prec_monthly.values
                            )
                            glac_mass_monthly = (
                                ds_glac.glac_mass_monthly.values
                            )
                        except:
                            glac_acc_monthly = np.full(
                                (1, len(time_values)), np.nan
                            )
                            glac_melt_monthly = np.full(
                                (1, len(time_values)), np.nan
                            )
                            ...
                            glac_prec_monthly = np.full(
                                (1, len(time_values)), np.nan
                            )
                            glac_mass_monthly = np.full(
                                (1, len(time_values)), np.nan
                            )
                        # get annual vars
                    ...
                    # if glacier output DNE in sim output file, create empty nan arrays to keep record of missing glaciers
                        ...
                        # monthly vars
                        glac_runoff_monthly = np.full(
                            (1, len(time_values)), np.nan
                        )
                        offglac_runoff_monthly = np.full(
                            (1, len(time_values)), np.nan
                        )
                        glac_acc_monthly = np.full(
                            (1, len(time_values)), np.nan
                        ...
                        # annual vars
                        glac_area_annual = np.full(
                            (1, year_values.shape[0]), np.nan
                        )
                        glac_mass_annual = np.full(
                            (1, year_values.shape[0]), np.nan
                        )
        ...
        for var in vars:
            ...
            if var == "glac_runoff_monthly":
                ...
                ds.glac_runoff_monthly.attrs["long_name"] = (
                    "glacier-wide runoff"
                )
            ...
            # offglac_runoff_monthly
                ...
                ds.offglac_runoff_monthly.attrs["long_name"] = (
                    "off-glacier-wide runoff"
                )
                ds.offglac_runoff_monthly.attrs["units"] = "m3"
                ds.offglac_runoff_monthly.attrs["temporal_resolution"] = (
                    "monthly"
                )
            ...
            # glac_acc_monthly
                ...
                ds.glac_acc_monthly.attrs["comment"] = (
                    "only the solid precipitation"
                )
            ...
            # glac_refreeze_monthly
                ...
                ds.glac_refreeze_monthly.attrs["temporal_resolution"] = (
                    "monthly"
                )
            ...
            # glac_frontalablation_monthly
                ...
                ds.glac_frontalablation_monthly.attrs[
                    "temporal_resolution"
                ] = "monthly"
            ...
            # glac_massbaltotal_monthly
                ...
                ds.glac_massbaltotal_monthly.attrs["temporal_resolution"] = (
                    "monthly"
                )
            ...
            # glac_area_annual
                ...
                ds.glac_area_annual.attrs["comment"] = (
                    "area at start of the year"
                )
            ...
            if "annual" in var:
                ...
                ds.time.attrs["comment"] = (
                    "years referring to the start of each year"
                )
            elif "monthly" in var:
                ds.time.attrs["range"] = (
            ...
            if realizations[0]:
                ...
                ds.Climate_Model.attrs["long_name"] = (
        ...
        loop_end = time.time()
        print(
            f"Batch {nbatch} runtime:\t{np.round(loop_end - loop_start, 2)} seconds"
        )
    ...
    for vn in vns:
        if vn in vars:
            ...
            fn_merge_list_start = [
                int(f.split("-")[-2]) for f in fn_merge_list
            ]

            if len(fn_merge_list) > 0:
                fn_merge_list = [
                    x
                    for _, x in sorted(zip(fn_merge_list_start, fn_merge_list))
                ]
    ...
    for sce in scenarios:
        for reg in region:
            ...
            print(
                ...
                [
                    f"{name}={val}"
                    for name, val in zip(kwargs, list_packed_vars[-1])
                ],
            )
            i += 1
