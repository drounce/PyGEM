...
def run(list_packed_vars):
    ...
    if args.option_bias_adjustment == 0 or gcm_name == args.ref_gcm_name:
        if pygem_prms["climate"]["gcm_wateryear"] == "hydro":
            ...
            dates_cn = "year"
        sim_idx_start = (
            dates_table_full[dates_cn].to_list().index(args.gcm_startyear)
        )
    ...
    if pygem_prms["mb"]["option_ablation"] != 2:
        ...
        ref_tempstd = np.zeros(
            (main_glac_rgi.shape[0], dates_table_ref.shape[0])
        )
    ...
    elif pygem_prms["mb"]["option_ablation"] == 2 and args.ref_gcm_name in [
        "ERA5"
    ]:
    ...
    for glac in range(main_glac_rgi.shape[0]):
        ...
        try:
            ...
            if (fls is not None) and (glacier_area_km2.sum() > 0):
                ...
                if args.option_calibration:
                    ...
                    if args.option_calibration == "MCMC":
                        if nsims == 1:
                            modelprms_all = {
                                "kp": [
                                    np.median(modelprms_all["kp"]["chain_0"])
                                ],
                                "tbias": [
                                    np.median(
                                        modelprms_all["tbias"]["chain_0"]
                                    )
                                ],
                                "ddfsnow": [
                                    np.median(
                                        modelprms_all["ddfsnow"]["chain_0"]
                                    )
                                ],
                                "ddfice": [
                                    np.median(
                                        modelprms_all["ddfice"]["chain_0"]
                                    )
                                ],
                                "tsnow_threshold": modelprms_all[
                                    "tsnow_threshold"
                                ],
                            ...
                            mcmc_sample_no = len(
                                modelprms_all["kp"]["chain_0"]
                            )
                            sims_burn = int(
                                args.mcmc_burn_pct / 100 * mcmc_sample_no
                            )
                            mp_spacing = int(
                                (mcmc_sample_no - sims_burn) / nsims
                            )
                            mp_idx_start = np.arange(
                                sims_burn, sims_burn + mp_spacing
                            )
                    ...
                    if (
                        ...
                        if rgiid in calving_rgiids:
                            ...
                            calving_k = calving_df.loc[
                                calving_idx, "calving_k"
                            ]
                        ...
                        if nsims == 1:
                            ...
                            while (
                                not abs(
                                    np.median(calving_k_values) - calving_k
                                )
                                < 0.001
                            ):
                                calving_k_values = (
                                ...
                                calving_k_values[calving_k_values < 0.001] = (
                                    0.001
                                )
                                calving_k_values[calving_k_values > 5] = 5
                            ...
                            assert (
                                abs(np.median(calving_k_values) - calving_k)
                                < 0.001
                    ...
                    calving_k = (
                        np.zeros(nsims)
                        + pygem_prms["sim"]["params"]["calving_k"]
                    )
                    calving_k_values = calving_k
                ...
                if args.option_dynamics in [
                    ...
                    if (
                        ...
                        cfg.PARAMS["cfl_number"] = pygem_prms["sim"][
                            "oggm_dynamics"
                        ]["cfl_number"]
                    else:
                        cfg.PARAMS["cfl_number"] = pygem_prms["sim"][
                    ...
                    if args.use_reg_glena:
                        ...
                        glena_O1regions = [
                            int(x) for x in glena_df.O1Region.values
                        ]
                ...
                if pygem_prms["climate"]["gcm_wateryear"] == "hydro":
                    annual_columns = np.unique(
                        dates_table["wateryear"].values
                ...
                for n_iter in range(nsims):
                    ...
                    if args.option_dynamics is not None:
                        ...
                        if (
                            ...
                            or not pygem_prms["setup"][
                                "include_frontalablation"
                            ]
                        ):
                        ...
                        th = cls["hgt"][-1]
                        vmin, vmax = cfg.PARAMS[
                            "free_board_marine_terminating"
                        ]
                        water_level = utils.clip_scalar(
                            0, th - vmax, th - vmin
                        )
                    ...
                    if args.option_dynamics == "OGGM":
                        ...
                        try:
                            ...
                            ev_model.mb_model.glac_wide_area_annual[-1] = (
                                diag.area_m2[-1]
                            )
                            ...
                            if "Glacier exceeds domain boundaries" in repr(e):
                                ...
                                ev_model.mb_model.glac_wide_frontalablation = ev_model.mb_model.glac_bin_frontalablation.sum(
                                    0
                                )
                    ...
                    # Mass redistribution model
                        ...
                        years = np.arange(
                            args.gcm_startyear, args.gcm_endyear + 1
                        )
                        mb_all = []
                    ...
                    if successful_run:
                        if args.option_dynamics is not None:
                            ...
                            area_initial = mbmod.glac_bin_area_annual[
                                :, 0
                            ].sum()
                            mb_mwea_diag = (
                                (
                                    diag.volume_m3.values[-1]
                                    - diag.volume_m3.values[0]
                                )
                                / area_initial
                                / nyears
                                * pygem_prms["constants"]["density_ice"]
                            ...
                            if debug:
                                ...
                                print(
                                    "  vol init  [Gt]:",
                                    np.round(
                                        diag.volume_m3.values[0] * 0.9 / 1e9, 5
                                    ),
                                )
                                print(
                                    "  vol final [Gt]:",
                                    np.round(
                                        diag.volume_m3.values[-1] * 0.9 / 1e9,
                                        5,
                                    ),
                                )
                                print(
                                ...
                                print(
                                    "  mb [mwea]:", np.round(mb_mwea_diag, 2)
                                )
                                print(
                                    "  mb_mbmod [mwea]:",
                                    np.round(mb_mwea_mbmod, 2),
                                )
                        ...
                        output_glac_temp_monthly[:, n_iter] = (
                            mbmod.glac_wide_temp
                        )
                        output_glac_prec_monthly[:, n_iter] = (
                            mbmod.glac_wide_prec
                        )
                        ...
                        output_glac_ELA_annual[:, n_iter] = (
                            mbmod.glac_wide_ELA_annual
                        )
                        output_offglac_prec_monthly[:, n_iter] = (
                            mbmod.offglac_wide_prec
                        )
                        ...
                        if output_glac_bin_icethickness_annual is None:
                            ...
                            if ev_model is not None:
                                fl_dx_meter = getattr(
                                    ev_model.fls[0], "dx_meter", None
                                )
                                fl_widths_m = getattr(
                                    ev_model.fls[0], "widths_m", None
                                )
                                fl_section = getattr(
                                    ev_model.fls[0], "section", None
                                )
                            else:
                                fl_dx_meter = getattr(
                                    nfls[0], "dx_meter", None
                                )
                                fl_widths_m = getattr(
                                    nfls[0], "widths_m", None
                                )
                                fl_section = getattr(nfls[0], "section", None)
                            if (
                                fl_section is not None
                                and fl_widths_m is not None
                            ):
                                # thickness
                            ...
                            output_glac_bin_acc_monthly = (
                                output_glac_bin_acc_monthly_sim[
                                    :, :, np.newaxis
                                ]
                            )
                            # refreeze
                            ...
                            output_glac_bin_melt_monthly_sim = (
                                mbmod.glac_bin_melt
                            )
                ...
                if count_exceed_boundary_errors < nsims:
                    ...
                    if args.export_all_simiters and nsims > 1:
                        ...
                        for n_iter in range(nsims):
                            ...
                            ].values[0, :] = output_offglac_runoff_monthly[
                                :, n_iter
                            ]
                            if args.export_extra_vars:
                                ...
                                output_ds_all_stats[
                                    "glac_prec_monthly"
                                ].values[0, :] = output_glac_prec_monthly[
                                    :, n_iter
                                ]
                                ...
                                ].values[0, :] = output_glac_refreeze_monthly[
                                    :, n_iter
                                ]
                                output_ds_all_stats[
                                    "glac_melt_monthly"
                                ].values[0, :] = output_glac_melt_monthly[
                                    :, n_iter
                                ]
                                output_ds_all_stats[
                                ...
                                ].values[0, :] = output_glac_snowline_monthly[
                                    :, n_iter
                                ]
                                output_ds_all_stats[
                                ...
                                ].values[0, :] = output_offglac_prec_monthly[
                                    :, n_iter
                                ]
                                output_ds_all_stats[
                                    "offglac_melt_monthly"
                                ].values[0, :] = output_offglac_melt_monthly[
                                    :, n_iter
                                ]
                                output_ds_all_stats[
                    ...
                    if args.export_extra_vars:
                        ...
                        output_glac_frontalablation_monthly_stats = (
                            calc_stats_array(
                                output_glac_frontalablation_monthly
                            )
                        )
                        output_glac_massbaltotal_monthly_stats = (
                            calc_stats_array(output_glac_massbaltotal_monthly)
                        )
                        output_glac_snowline_monthly_stats = calc_stats_array(
                            output_glac_snowline_monthly
                        )
                        output_glac_mass_change_ignored_annual_stats = (
                            calc_stats_array(
                                output_glac_mass_change_ignored_annual
                            )
                        )
                        ...
                        output_offglac_refreeze_monthly_stats = (
                    ...
                    if nsims > 1:
                        ...
                        ].values[0, :] = output_offglac_runoff_monthly_stats[
                            :, 1
                        ]
                        if args.export_extra_vars:
                            output_ds_all_stats[
                                "glac_temp_monthly_mad"
                            ].values[0, :] = output_glac_temp_monthly_stats[
                                :, 1
                            ]
                            output_ds_all_stats[
                                "glac_prec_monthly_mad"
                            ].values[0, :] = output_glac_prec_monthly_stats[
                                :, 1
                            ]
                            ...
                            ].values[0, :] = output_glac_melt_monthly_stats[
                                :, 1
                            ]
                            output_ds_all_stats[
                            ...
                            ].values[0, :] = output_offglac_prec_monthly_stats[
                                :, 1
                            ]
                            output_ds_all_stats[
                                "offglac_melt_monthly_mad"
                            ].values[0, :] = output_offglac_melt_monthly_stats[
                                :, 1
                            ]
                            output_ds_all_stats[
                    ...
                    if (
                        ...
                        > pygem_prms["sim"]["out"][
                            "export_binned_area_threshold"
                        ]
                    ):
                        # Distance from top of glacier downglacier
                        output_glac_bin_dist = (
                            np.arange(nfls[0].nx) * nfls[0].dx_meter
                        )
            ...
            with open(fail_fp + txt_fn_fail, "w") as text_file:
                text_file.write(
                    glacier_str + f" failed to complete simulation: {err}"
                )
            ...
            for realization in realizations:
                for count, glac_no_lst in enumerate(glac_no_lsts):
                    list_packed_vars.append(
                        [count, glac_no_lst, gcm_name, realization]
                    )
        else:
            for count, glac_no_lst in enumerate(glac_no_lsts):
                list_packed_vars.append(
                    [count, glac_no_lst, gcm_name, realizations]
                )
