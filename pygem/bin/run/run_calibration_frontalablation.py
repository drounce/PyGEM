...
def reg_calving_flux(
    ...
    if pygem_prms["mb"]["option_ablation"] == 2 and args.ref_gcm_name in [
        "ERA5"
    ]:
    ...
    for nglac in np.arange(main_glac_rgi.shape[0]):
        ...
        try:
            ...
            debris.debris_binned(
                gdir, fl_str="inversion_flowlines", ignore_debris=True
            )
        except:
            fls = None
        ...
        if (fls is not None) and (glacier_area.sum() > 0):
            ...
            if pygem_prms["sim"]["oggm_dynamics"]["use_reg_glena"]:
                ...
                glena_idx = np.where(
                    glena_df.O1Region == glacier_rgi_table.O1Region
                )[0][0]
            ...
            try:
                ...
                if gdir.is_tidewater:
                    ...
                    out_calving_forward["calving_front_thick"] = thick[
                        last_idx
                    ]

                    # Record in dataframe
                    output_df.loc[nglac, "calving_flux_Gta"] = calving_flux_Gta
                    output_df.loc[nglac, "calving_thick"] = (
                        out_calving_forward["calving_front_thick"]
                    )
                    ...
                    if args.verbose or debug:
                        ...
                        print(
                            ...
                            np.round(
                                out_calving_forward["calving_front_thick"], 1
                            ),
                        )
                        print(
                ...
                if gdir.is_tidewater:
                    ...
                    if args.verbose or debug:
                        ...
                        print(
                            "    calving front thickness [m]:",
                            np.round(
                                out_calving_forward["calving_front_thick"], 0
                            ),
                        )
                        print(
                            "    calving flux model [Gt/yr]:",
            ...
            if calc_mb_geo_correction:
                ...
                if last_yr_idx == mbmod.glac_bin_area_annual.shape[1] - 1:
                    last_yr_idx = -2
                bin_last_idx = np.where(
                    mbmod.glac_bin_area_annual[:, last_yr_idx] > 0
                )[0][-1]
                bin_area_lost = (
                ...
                if (
                    mb_mwea_fa_asl_geo_correction
                    > mb_mwea_fa_asl_geo_correction_max
                ):
                    mb_mwea_fa_asl_geo_correction = (
                        mb_mwea_fa_asl_geo_correction_max
                    )
    ...
    if fa_glac_data_reg is None:
        ...
        reg_calving_gta_obs_good = fa_glac_data_reg[
            frontal_ablation_Gta_cn
        ].sum()
...
def run_opt_fa(
    ...
    if reg_calving_gta_mod < reg_calving_gta_obs:
        ...
        while (
            ...
            and (
                ...
                and np.abs(reg_calving_gta_mod - reg_calving_gta_obs)
                > fa_threshold
            )
        ):
        ...
        if args.verbose:
            ...
            print(
                "fa thres:", np.abs(reg_calving_gta_mod - reg_calving_gta_obs)
            )
    ...
    if not np.isnan(output_df.loc[0, "calving_flux_Gta"]):
        ...
        if (
            ...
            / reg_calving_gta_obs
            < perc_threshold_agreement
            or np.abs(reg_calving_gta_mod_bndhigh - reg_calving_gta_obs)
            < fa_threshold
        ):
        ...
        # Check if lower bound causes good fit
            ...
            / reg_calving_gta_obs
            < perc_threshold_agreement
            or np.abs(reg_calving_gta_mod_bndlow - reg_calving_gta_obs)
            < fa_threshold
        ):
            calving_k = calving_k_bndlow
            # Re-run the regional frontal ablation estimates
            ...
            while (
                ...
                if args.verbose:
                    print("  calving_k:", np.round(calving_k, 4))
                    print(
                        "  fa_data  [Gt/yr]:", np.round(reg_calving_gta_obs, 4)
                    )
                    print(
                        "  fa_model [Gt/yr] :",
                        np.round(reg_calving_gta_mod, 4),
                    )
...
def calib_ind_calving_k(
    ...
    for reg in [regions]:
        ...
        # Regional data
        fa_glac_data_reg = fa_glac_data.loc[
            fa_glac_data["O1Region"] == reg, :
        ].copy()
        ...
        if not os.path.exists(output_fp + output_fn) or overwrite:
            ...
            output_df_all = pd.DataFrame(
                np.zeros((main_glac_rgi.shape[0], len(output_cns))),
                columns=output_cns,
            )
            ...
            fa_obs_dict = dict(
                zip(
                    ...
                    fa_glac_data_reg[frontal_ablation_Gta_cn],
                )
            )
            ...
            output_df_all["fa_gta_obs"] = output_df_all["RGIId"].map(
                fa_obs_dict
            )
            ...
            output_df_all["area_km2"] = output_df_all["RGIId"].map(
                rgi_area_dict
            )
            ...
            for nglac, rgiid in enumerate(list(output_df_all.RGIId)):
                ...
                output_df_all.loc[nglac, "mb_total_gta_obs"] = (
                    fa_glac_data_reg.loc[fa_idx, "Romain_gta_mbtot"]
                )
                output_df_all.loc[nglac, "mb_clim_gta_obs"] = (
                    fa_glac_data_reg.loc[fa_idx, "Romain_gta_mbclim"]
                )
                output_df_all.loc[nglac, "mb_total_mwea_obs"] = (
                    fa_glac_data_reg.loc[fa_idx, "Romain_mwea_mbtot"]
                )
                output_df_all.loc[nglac, "mb_clim_mwea_obs"] = (
                    fa_glac_data_reg.loc[fa_idx, "Romain_mwea_mbclim"]
                )
            ...
            if output_df_badmbclim.shape[0] > 0:
                ...
                for nglac, rgiid in enumerate(list(output_df_all.RGIId)):
                    if rgiid in rgiids_toopos:
                        ...
                        mb_total_gta = output_df_all.loc[
                            nglac, "mb_total_gta_obs"
                        ]
            ...
            failed_glacs = []
            for nglac in np.arange(main_glac_rgi.shape[0]):
                glacier_str = "{0:0.5f}".format(
                    main_glac_rgi.loc[nglac, "RGIId_float"]
                )
                try:
                    # Reset bounds
                    ...
                    if verbose:
                        print(
                            "  fa_data  [Gt/yr]:",
                            np.round(reg_calving_gta_obs, 4),
                        )
                        print(
                    ...
                    if run_opt:
                        ...
                        fa_glac_data_ind_high.loc[0, "fa_gta_obs"] = (
                            fa_gta_obs_high
                        )
                        ...
                        if verbose:
                            print(
                                ...
                                "fa_mod high:",
                                np.round(
                                    output_df.loc[0, "calving_flux_Gta"], 4
                                ),
                            )
                        ...
                        if fa_gta_obs_low < 0:
                            ...
                            fa_glac_data_ind_low.loc[0, "fa_gta_obs"] = (
                                fa_gta_obs_low
                            )
                            calving_k_bndhigh_lower = (
                                np.copy(calving_k_med) + 0.01
                            )
                            calving_k_start = np.copy(calving_k_med)
                            output_df, calving_k = run_opt_fa(
                                main_glac_rgi_ind,
                        ...
                        output_df_all.loc[nglac, "calving_k_nmad"] = (
                            calving_k_nmad
                        )
                except:
            ...
            if len(failed_glacs) > 0:
                with open(
                    output_fp + output_fn[:-4] + "-failed.txt", "w"
                ) as f:
        ...
        if len(rgiids_good) > 0:
            ...
            s_byarea[(glac_area_all >= 10) & (glac_area_all < 100)] = s_sizes[
                1
            ]
            s_byarea[(glac_area_all >= 100) & (glac_area_all < 1000)] = (
                s_sizes[2]
            )
            ...
            ax[0, 1].set_xticks(
                np.arange(0, np.max([1, vn_bins.max()]) + 0.1, 1)
            )
            ...
            if hist.max() < 40:
                y_major_interval = 5
                y_max = (
                    np.ceil(hist.max() / y_major_interval) * y_major_interval
                )
                ax[0, 1].set_yticks(
            ...
            fig_fullfn = (
                output_fp
                + str(reg)
                + "-frontalablation_glac_compare-cal_ind-good.png"
            )
        ...
        for nglac, rgiid in enumerate(output_df_all.RGIId):
            if rgiid in rgiids_bndhigh:
                ...
                main_glac_rgi_ind = main_glac_rgi.loc[
                    main_glac_rgi.RGIId == rgiid, :
                ]
                main_glac_rgi_ind.reset_index(inplace=True, drop=True)
                fa_glac_data_ind = fa_glac_data_reg.loc[
        ...
        if reg in [1, 3, 4, 5, 7, 9, 17]:
            ...
            rgiids_missing = [
                x for x in rgiids_all if x not in rgiids_processed
            ]
            ...
            if not os.path.exists(output_fp + output_fn_missing) or overwrite:
                ...
                output_df_all["O1Region"] = [
                    ...
                    for x in output_df_all.RGIId
                ]

                # Update mass balance data
                output_df_missing = pd.DataFrame(
                    np.zeros(
                        (len(rgiids_missing), len(output_df_all.columns))
                    ),
                ...
                rgi_area_dict = dict(
                    zip(
                        main_glac_rgi_missing.RGIId, main_glac_rgi_missing.Area
                    )
                )
                ...
                rgi_mbobs_dict = dict(
                    zip(mb_data["rgiid"], mb_data["mb_mwea"])
                )
                output_df_missing["mb_clim_mwea_obs"] = output_df_missing[
                    "RGIId"
                ].map(rgi_mbobs_dict)
                ...
                calving_k_med = np.median(
                    output_df_all.loc[
                        output_df_all["O1Region"] == reg, "calving_k"
                    ]
                )
                failed_glacs = []
                for nglac, rgiid in enumerate(rgiids_missing):
                    ...
                    try:
                        ...
                        mb_clim_reg_95 = (
                            mb_clim_reg_avg + 1.96 * mb_clim_reg_std
                        )
                        if verbose:
                            print(
                                "mb_clim (raw):",
                            ...
                            print(
                                "mb_total (95% min):",
                                np.round(mb_clim_reg_3std_min, 2),
                            )
                        ...
                        if mb_clim_fa_corrected > mb_clim_reg_95:
                            ...
                            # Record bounds
                            output_df_missing.loc[
                                nglac, "calving_flux_Gta_bndlow"
                            ] = reg_calving_gta_mod_bndlow
                            output_df_missing.loc[
                                nglac, "calving_flux_Gta_bndhigh"
                            ] = reg_calving_gta_mod_bndhigh
                        ...
                        pass
                # Export
                output_df_missing.to_csv(
                    output_fp + output_fn_missing, index=False
                )
                ...
                for nglac, rgiid in enumerate(rgiids_missing):
                    try:
                        ...
                        if mb_clim_mwea < mb_clim_reg_95:
                            ...
                            # Calibrate frontal ablation based on fa_mwea_max
                            #  i.e., the maximum frontal ablation that is consistent with reasonable mb_clim
                            fa_mwea_max = mb_fa_mwea - (
                                mb_clim_mwea - mb_clim_reg_95
                            )
                            ...
                            if bndhigh_good and bndlow_good:
                                ...
                                if verbose:
                                    ...
                                    print(
                                        "mb_fa_mwea:", np.round(mb_fa_mwea, 2)
                                    )
                                    print(
                                        "mb_clim (reg 95%):",
                                        np.round(mb_clim_reg_95, 2),
        ...
        with np.errstate(all="ignore"):
            plot_max_raw = np.max(
                [
                    output_df_all.calving_flux_Gta.max(),
                    output_df_all.fa_gta_obs.max(),
                ]
            )
            plot_max = 10 ** np.ceil(np.log10(plot_max_raw))
            ...
            plot_min_raw = np.max(
                [
                    output_df_all.calving_flux_Gta.min(),
                    output_df_all.fa_gta_obs.min(),
                ]
            )
            plot_min = 10 ** np.floor(np.log10(plot_min_raw))
        if plot_min < 1e-3:
        ...
        vn_bins = np.arange(
            0, np.max([1, output_df_all.calving_k.max()]) + 0.1, 0.1
        )
        hist, bins = np.histogram(
        ...
        fig.set_size_inches(6, 6)
        fig_fullfn = (
            output_fp + str(reg) + "-frontalablation_glac_compare-cal_ind.png"
        )
        fig.savefig(fig_fullfn, bbox_inches="tight", dpi=300)
        plt.close(fig)
...
def merge_ind_calving_k(
    ...
    output_reg_fns = sorted(
        glob.glob(f"{output_fp}/*-frontalablation_cal_ind.csv")
    )
    ...
    for nreg, output_fn_reg in enumerate(output_reg_fns):
        ...
        if os.path.exists(output_fp + output_fn_reg_missing):
            ...
            if os.path.exists(output_fp + output_fn_reg_missing_v2):
                ...
                output_df_reg_missing = pd.read_csv(
                    output_fp + output_fn_reg_missing
                )

            if "calving_k_nmad" not in list(output_df_reg_missing.columns):
                output_df_reg_missing["calving_k_nmad"] = 0

            output_df_all = pd.concat(
                [output_df_all, output_df_reg_missing], axis=0
            )

    output_df_all.to_csv(output_fp + merged_calving_k_fn, index=0)
    if verbose:
        print(
            f"Merged calving calibration exported: {output_fp + merged_calving_k_fn}"
        )
...
def update_mbdata(
    ...
    for nglac, rgiid in enumerate(fa_glac_data.RGIId):
        ...
        if O1region in regions:
            ...
            mb_idx = mb_rgiids.index(rgiid)
            mb_data.loc[mb_idx, "mb_mwea"] = fa_glac_data.loc[
                nglac, "mb_total_mwea"
            ]
...
def plot_calving_k_allregions(output_fp=""):
    ...
    for nax, ax in enumerate([ax1, ax2, ax3, ax4, ax5, ax6, ax7, ax8, ax9]):
        ...
        ax.tick_params(
            axis="both", which="major", direction="inout", right=True
        )
...
def main():
    ...
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="Flag for verbose"
    )
