...
def export_priors(priors_df_single, reg, regO2, priors_reg_outpath=""):
    ...
    priors_df = priors_df.sort_values(
        ["O1Region", "O2Region"], ascending=[True, True]
    )
...
def plot_hist(main_glac_rgi_subset, fig_fp, reg, regO2=""):
    ...
    fig_fn = (
        ...
        + "-"
        + str(regO2)
        + "_hist_mcmc_priors.png".replace("-_", "_")
    )
...
def plot_reg_priors(main_glac_rgi, priors_df, reg, rgi_regionsO2, fig_fp):
    ...
    for nreg, regO2 in enumerate(rgi_regionsO2):
        ...
        kp_values = main_glac_rgi.loc[
            main_glac_rgi["O2Region"] == regO2, "kp"
        ].values
    ...
    fig.text(
        0.5, 0.04, r"$T_{bias}$ ($^\circ$C)", va="center", ha="center", size=12
    )
    fig.set_size_inches(6, 6)
    fig.savefig(
...
def run(
    ...
    modelprms_fp = (
        pygem_prms["root"] + "/Output/calibration/" + str(reg).zfill(2) + "/"
    )
    # Load glaciers
    glac_list = [
        x.split("-")[0]
    ...
    for nglac, rgino_str in enumerate(list(main_glac_rgi.rgino_str.values)):
        glac_str = (
            str(int(rgino_str.split(".")[0])) + "." + rgino_str.split(".")[1]
        )
    ...
    if reg not in [19]:
        ...
        for regO2 in rgi_regionsO2:
            ...
            priors_df = export_priors(
                priors_df_single, reg, regO2, priors_reg_outpath
            )
