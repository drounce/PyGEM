...
class oib:
    def __init__(self, rgi6id="", rgi7id=""):
        ...
        self.rgi7_6_df = pd.read_csv(
            f"{self.oib_datpath}/../oibak_rgi6_rgi7_ids.csv"
        )
        self.rgi7_6_df["rgi7id"] = (
            self.rgi7_6_df["rgi7id"].str.split("RGI2000-v7.0-G-").str[1]
        )
        self.rgi7_6_df["rgi6id"] = (
            self.rgi7_6_df["rgi6id"].str.split("RGI60-").str[1]
        )
        self.rgi6id = rgi6id
        self.rgi7id = rgi7id
        self.name = None
    ...
    def _rgi6torgi7id(self, debug=False):
        ...
        if len(rgi7id) == 1:
            ...
            raise IndexError(
                f"More than one matching RGI7Id for {self.rgi6id}"
            )
    ...
    def _rgi7torgi6id(self, debug=False):
        ...
        if len(rgi6id) == 1:
            ...
            raise IndexError(
                f"More than one matching RGI6Id for {self.rgi7id}"
            )
    ...
    def _date_check(self, dt_obj):
        ...
        if dt_obj.day < dim // 2:
            dt_obj_ = datetime.datetime(
                year=dt_obj.year, month=dt_obj.month, day=1
            )
        else:
            dt_obj_ = datetime.datetime(
                year=dt_obj.year, month=dt_obj.month + 1, day=1
            )
        return dt_obj_
    ...
    def _load(self):
        ...
        """
        oib_fpath = glob.glob(
            f"{self.oib_datpath}/diffstats5_*{self.rgi7id}*.json"
        )
        if len(oib_fpath) == 0:
            return
        else:
    ...
    def _parsediffs(self, filter_count_pctl=10, debug=False):
        ...
        for ssn in seasons:
            for yr in list(self.oib_dict[ssn].keys()):
                ...
                counts = np.asarray(
                    self.oib_dict[ssn][yr]["bin_vals"]["bin_count_vec"]
                )
        ...
        self.bin_centers = (
            np.asarray(
                self.oib_dict[ssn][list(self.oib_dict[ssn].keys())[0]][
                    "bin_vals"
                ]["bin_start_vec"]
            )
            + np.asarray(
    ...
    def _rebin(self, agg=100):
        if agg:
            ...
            nbins = int(
                np.ceil((self.bin_centers[-1] - self.bin_centers[0]) // agg)
            )
    ...
    def _elevchange_to_masschange(
        ...
        if self.dbl_diffs["dh"] is not None:
            ...
            conversion_factor[np.where(self.bin_centers < ela)] = (
                density_ablation
            )
            conversion_factor[np.where(self.bin_centers >= ela)] = (
