...
if "debris_hd" not in cfg.BASENAMES:
    cfg.BASENAMES["debris_hd"] = (
        "debris_hd.tif",
        "Raster of debris thickness data",
    )
...
def debris_to_gdir(
    ...
    assert os.path.exists(debris_dir), (
        "Error: debris directory does not exist."
    )
    ...
    glac_str_nolead = (
        str(int(gdir.rgi_region))
        + "."
        + gdir.rgi_id.split("-")[1].split(".")[1]
    )
...
def debris_binned(gdir, ignore_debris=False, fl_str="inversion_flowlines"):
    ...
    if flowlines is not None:
        ...
        if (
            os.path.exists(gdir.get_filepath("debris_hd"))
            and ignore_debris == False
        ):
            ...
            for nbin in np.arange(0, len(z_bin_edges) - 1):
                ...
                bin_idx = np.where(
                    (topo_onglac < bin_max) & (topo_onglac >= bin_min)
                )[0]
