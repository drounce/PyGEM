...
if input.option_gcm_downscale == 1:  
    ...
    main_glac_gcmtemp_all = np.genfromtxt(input.gcm_filepath_var + input.gcmtemp_filedict[input.rgi_regionsO1[0]], 
                                          delimiter=',')
    main_glac_gcmprec_all = np.genfromtxt(input.gcm_filepath_var + input.gcmprec_filedict[input.rgi_regionsO1[0]], 
                                          delimiter=',')
    ...
    if input.rgi_glac_number == 'all':
        ...
        main_glac_gcmprec = main_glac_gcmprec_all
        main_glac_gcmelev = main_glac_gcmelev_all
        if input.option_lapserate_fromgcm == 1:
            main_glac_gcmlapserate = main_glac_gcmlapserate_all
    else:
        main_glac_gcmtemp = np.zeros((main_glac_rgi.shape[0], dates_table.shape[0]))
        main_glac_gcmprec = np.zeros((main_glac_rgi.shape[0], dates_table.shape[0]))
        main_glac_gcmelev = np.zeros((main_glac_rgi.shape[0]))
        if input.option_lapserate_fromgcm == 1:
            main_glac_gcmlapserate = np.zeros((main_glac_rgi.shape[0], dates_table.shape[0]))
        # Select climate data for each glacier using O1Index
        for glac in range(main_glac_rgi.shape[0]):
            main_glac_gcmtemp[glac,:] = main_glac_gcmtemp_all[main_glac_rgi.loc[glac,'O1Index'],:]
            main_glac_gcmprec[glac,:] = main_glac_gcmprec_all[main_glac_rgi.loc[glac,'O1Index'],:]
            main_glac_gcmelev[glac] = main_glac_gcmelev_all[main_glac_rgi.loc[glac,'O1Index']]
            if input.option_lapserate_fromgcm == 1:
                main_glac_gcmlapserate[glac,:] = main_glac_gcmlapserate_all[main_glac_rgi.loc[glac,'O1Index'],:]
...
#        glac = 3472
        ...
        if np.isnan(main_glac_rgi.loc[glac,input.massbal_colname]) == False:
            ...
            if massbal_difference > input.massbal_tolerance:
                ...
                massbal_difference = abs(glacier_rgi_table[input.massbal_colname] - glac_wide_massbaltotal_annual_avg)
                main_glac_massbal_compare[glac] = [glac_wide_massbaltotal_annual_avg, 
                                                   glacier_rgi_table.loc[input.massbal_colname], 
...
#    for glac in [1]:
        ...
        if np.isnan(main_glac_calmassbal[glac,0]) == False:
            ...
            # Set new output 
            output_glac_wide_massbaltotal = np.zeros((grid_modelparameters.shape[0], glacier_gcm_temp.shape[0] - 
                                                      input.spinupyears*12))
            output_glac_wide_runoff = np.zeros((grid_modelparameters.shape[0], glacier_gcm_temp.shape[0] - 
                                                input.spinupyears*12))
            output_glac_wide_snowline = np.zeros((grid_modelparameters.shape[0], glacier_gcm_temp.shape[0] - 
                                                  input.spinupyears*12))
            output_glac_wide_snowpack = np.zeros((grid_modelparameters.shape[0], glacier_gcm_temp.shape[0] - 
                                                  input.spinupyears*12))
            output_glac_wide_area_annual = np.zeros((grid_modelparameters.shape[0], annual_columns.shape[0] - 
                                                     input.spinupyears + 1))
            output_glac_wide_volume_annual = np.zeros((grid_modelparameters.shape[0], annual_columns.shape[0] - 
                                                       input.spinupyears + 1))
            output_glac_wide_ELA_annual = np.zeros((grid_modelparameters.shape[0], annual_columns.shape[0] - 
                                                    input.spinupyears))
            ...
            for grid_round in range(grid_modelparameters.shape[0]):
                ...
                glac_wide_ELA_annual[glac_wide_ELA_annual > 0] = (
                        elev_bins[glac_wide_ELA_annual[glac_wide_ELA_annual > 0]] - input.binsize/2)
            ...
            # Netcdf file path and name
            filename = (input.calibrationnetcdf_filenameprefix + str(regionO1_number) + '_' + str(strftime("%Y%m%d")) 
                        + '.nc')
            fullfile = input.output_filepath + filename
