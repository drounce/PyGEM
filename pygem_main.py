...
for glac in [0]:
    ...
    # FROM HERE DOWN GOING INTO FUNCTION!
    ...
    glac_bin_temp = massbalance.downscaletemp2bins(glacier_rgi_table, glacier_gcm_temp, glacier_gcm_elev, elev_bins, 
                                                   modelparameters)
    # Downscale the gcm precipitation [m] to each bin (includes solid and liquid precipitation)
    glac_bin_precsnow = massbalance.downscaleprec2bins(glacier_rgi_table, glacier_gcm_prec, glacier_gcm_elev, elev_bins, 
                                                       modelparameters)
    # Compute accumulation [m w.e.] and precipitation [m] for each bin
    glac_bin_prec, glac_bin_acc = massbalance.accumulationbins(glac_bin_temp, glac_bin_precsnow, modelparameters)
...
#    for step in range(0,12):
        ...
        if (step + 1) % annual_divisor == 0:
            ...
            surfacetype, firnline_idx = massbalance.surfacetypebinsannual(surfacetype, glac_bin_massbalclim_annual, 
                                                                          year_index)
            ...
            if input.option_calibration == 0:
                ...
                glacier_area_t1, icethickness_t1, width_t1 = massbalance.massredistributionHuss(
                        glacier_area_t0, icethickness_t0, width_t0, glac_bin_massbalclim_annual, year_index, 
                        glac_idx_initial)
