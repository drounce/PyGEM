...
gcm_temp_pressurelevels_filename = 'HMA_ERAInterim_temp_pressurelevels_1994.nc'
...
#print(data[variablename].sel(time=0, latitude=0, longitude=0))
...
# Generate dates using date_range function
dates = pd.DataFrame({'date' : pd.date_range(startdate, enddate, freq='MS')})
dates = dates['date'].astype(datetime)

netcdf_filenameprefix = 'HMA_Regions13_14_15_ERAInterim_lapserates_1979_2017.nc'
netcdf_filepath = '../Output/'
# Create netcdf lapse rates as a function of latitude, longitude, and time
filename = netcdf_filenameprefix
...
for year in range(startyear,endyear+1):
    print(year)
#for year in [1979]:
    filenameyear = gcm_filepath_var + 'HMA_EraInterim_temp_pressurelevels_' + str(year) + '.nc'
    datayear = nc.Dataset(filenameyear,'r')
    count = count + 1
    for lat in range(0,latitude[:].shape[0]):
        for lon in range(0,longitude[:].shape[0]):
#    for lat in [0]:
#        for lon in [0]:
#            print(data['latitude'].values[lat], data['longitude'].values[lon])
            A = data[variablename].isel(level=range(A_elev_idx_max,A_elev_idx_min+1), latitude=lat, longitude=lon).values
            A_slope = ((A_elev[A_elev_idx_max:A_elev_idx_min+1]*A).mean(axis=1) - A_elev[A_elev_idx_max:A_elev_idx_min+1].mean()*A.mean(axis=1)) / ((A_elev[A_elev_idx_max:A_elev_idx_min+1]**2).mean() - (A_elev[A_elev_idx_max:A_elev_idx_min+1].mean())**2)
            lapserate[lat,lon,12*(count-1):12*count] = A_slope
            # Takes roughly 4 minutes per year to compute the lapse rate for each lat/lon combo in HMA
netcdf_output.close()

#A = data[variablename].isel(level=range(A_elev_idx_max,A_elev_idx_min+1), latitude=5, longitude=10).values
#A_slope = ((A_elev[A_elev_idx_max:A_elev_idx_min+1]*A).mean(axis=1) - A_elev[A_elev_idx_max:A_elev_idx_min+1].mean()*A.mean(axis=1)) / ((A_elev[A_elev_idx_max:A_elev_idx_min+1]**2).mean() - (A_elev[A_elev_idx_max:A_elev_idx_min+1].mean())**2)
#                      
