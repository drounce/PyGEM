...
Model parameters are specified via the configuration file (***~/PyGEM/config.yaml***), and a number of key parameters can also be passed to model scripts as command line arguments. The configuration file is [well documented](pygem_input_overview_target) and [sample files](https://drive.google.com/drive/folders/13kiU00Zz2swN5OzwXiWIQTj_JLEHnDgZ) are produced to support trial runs. 
...
```
{warning}
If you use a different file structure and do not update the file paths in the **~/PyGEM/config.yaml**, you will get an error and PyGEM will not run!
```
