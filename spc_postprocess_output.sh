...
for GCM_NAME in $GCM_NAMES_LST; do
  ...
  srun -N 1 -n 1 python run_postprocessing.py -gcm_name="$GCM_NAME_NOSPACE" -merge_batches=$MERGE_SWITCH -subset_vars=$SUBSET_SWITCH -vars_mon2annualseasonal=$MON2ANNUAL_SWITCH &
done
wait
